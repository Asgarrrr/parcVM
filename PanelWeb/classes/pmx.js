...
module.exports = class Proxmox {
     ...
     /**
      * @brief   Create a new instance of Proxmox
      * @param DB        { Object } Database object
      * @param socket    { Object } Socket object
      * @param apiToken  { String } API token
      * @param host      { String } Hostname of the Proxmox server
      * @param port      { Number } Port of the Proxmox server
      */
     ...
     async getNodes( ID ) {
         ...
         if ( ID )
             return data.filter( ( node ) => ID.includes( node.node ) );
      ...
      * @param   ID            { Number } VM ID (optional)
      * @param   allowTemplate { Boolean } Allow template VM (optional)
      * @return  { Array } List of VMs
      * @throws  { Error } If no VMID is provided
      */
     async getVMs( ID, allowTemplate = true ) {
         ...
         // —— Filter by ID
         if ( ID )
             resources = resources.filter( ( VM ) => VM.vmid === ID );
         ...
         await Promise.all( resources.map( async ( VM ) => {
             ...
             try {
                 ...
                 if ( VM.status === "running" ) {
                     ...
                     } else VM.network = this.cacheIP[ VM.vmid ].IP;
     ...
     /**
      * @brief   Create a new VM from a template
      * @param   templateID  { Number } ID of the template to use
      * @param   ID          { Number } ID of the VM to create
      * @param   name        { String } Name of the VM to create
      * @return  { String } Processed ID
      */
     ...
     /**
      * @brief   Get the next available VM ID
      * @return { Number } Next available VM ID
      */
     async getNextVMID( ) {
     ...
     /**
      * @brief   Push to the queue a new task
      * @param task  { Object } Task to push
      * @param args  { Object } Arguments to pass to the task
      * @param index { Number } Index of the task in the queue
      */
     ...
     /**
      * @brief   Get the list of currently running tasks and waiting tasks
      * @return {[][]}
      */
     getQueueTasks( ) {
     ...
     /**
      * @brief   Get the list of snapshots for a specific VM
      * @param   ID { Number }   ID of the VM to get the snapshots for
     ...
     /**
      * @brief   Create a new snapshot for a specific VM
      * @param   { Number } VMID         ID of the VM to create a snapshot for
      * @param   { String } snapname     Name of the snapshot to create
      * @param   { String } description  Description of the snapshot to create
      * @throws  { Error }               If the request failed
      * @return  { String }              Processed ID
      */
     ...
     /**
      * @brief   Delete a specific snapshot for a specific VM
      * @param   { Number } ID         ID of the VM to delete a snapshot for
      * @param   { String } snapshot   Name of the snapshot to delete
      * @return  { String }            Processed ID
      */
     ...
     /**
      * @brief   Create a new vnc console for a specific VM
      * @param   { Number } VMID     ID of the VM to create a vnc console for
      * @return
      */
     async vncproxy( VMID ) {
     ...
     /**
      * @brief   Get information about clusters and nodes
      * @return  { Object }      Clusters and nodes
      */
     async loadResources( ) {
